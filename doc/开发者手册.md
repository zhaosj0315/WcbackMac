...
当前目录包含 Mac 适配脚本。代码已形成两个运行面：

- 原 Windows/PyQt5 桌面端：入口为 `python main.py`。
- Mac 命令行导出链路：入口以 `scripts/mac_*.py` 为主。

Mac 版本不复用 Windows 的 `WeChat.exe`、`WeChatWin.dll`、注册表、`pywin32`、`pymem` 取 key 逻辑。Mac 版本使用以下链路：

1. `scripts/mac_probe_wechat.py` 探测微信进程和数据库目录。
2. `scripts/mac_auto_decrypt_export.py` 复制并重签微信副本，通过 LLDB 扫描 raw key。
3. `scripts/mac_decrypt_from_keys.py` 批量解密 WCDB 数据库。
4. `scripts/mac_export_messages.py`、`scripts/mac_export_html.py`、`scripts/mac_export_all.py` 导出聊天记录。

开发者调试命令：

```shell
python3 scripts/mac_probe_wechat.py
python3 scripts/mac_export_messages.py --db-dir app/Database/MacMsg --output data/mac_messages.csv --latest 100
python3 scripts/mac_export_html.py --db-dir app/Database/MacMsg --output data/html_export --limit 100
```

`scripts/wxdump_mac.py` 已作为统一 CLI 入口适配当前 Mac 脚本。复杂排障时仍建议直接运行专项脚本，以便保留更完整的日志。
