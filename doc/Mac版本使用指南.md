...
> 脚本启动微信副本后，会等待你确认“已登录且聊天列表已经加载完成”再执行 LLDB 扫描。  
> **注意：在回车开始扫描前，请至少点开 3-5 个聊天窗口/联系人资料页**，让对应数据库先被 WeChat 访问并把 key 物化到内存里；脚本会尝试扫描多个微信相关进程并合并结果。  
> 依赖 macOS 调试权限和微信版本，属实验性流程。如果失败，先运行 `python3 scripts/mac_probe_wechat.py` 检查 memory permission，再检查“系统设置 -> 隐私与安全性 -> 开发者工具”里是否已允许当前终端。
...
- 正序 / 倒序切换
- AI 推荐回复：打开会话后自动根据“最后一条对方消息”生成 3 条建议，参考该联系人的历史聊天风格
- 语音在线播放（wav）、视频在线播放、文件下载
- 点击顶部统计栏查看联系人列表和朋友圈

> AI 推荐回复需要本地 Ollama 已启动，且本机至少安装一个 `<=10B` 的模型；页面会自动过滤过大的模型，避免内存占用过高。
